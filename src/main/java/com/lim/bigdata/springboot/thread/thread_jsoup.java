...
import com.lim.bigdata.springboot.hash.hash_title;
...
public class thread_jsoup extends Thread {
    ...
    public void run() {
        try {
            ...
            Document doc = Jsoup.connect(url).header("Accept", "text/html, application/xhtml+xml,*/*").header("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; rv:11.0) like Gecko").header("Accept-Encoding", "gzip,delate").header("Accept-Language", "ko").header("Connection", "Keep-Alive").get();
